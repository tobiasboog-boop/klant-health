...
def save_powerbi_cache(file_bytes: bytes, validated_df=None) -> bool:
    """Sla geüpload Power BI Excel op als parquet in RAAS. Returns True bij succes."""
    try:
        from db import save_powerbi_cache as _save
        return _save(file_bytes, validated_df=validated_df)
    except Exception:
        return False


def load_powerbi_data():
    """Laad Power BI data uit RAAS.
    Returns (DataFrame, source_label, status)."""
    try:
        from db import load_powerbi_cache as _load
        return _load()
    except Exception:
...
def load_klantreis_fasen() -> dict:
    """Laad klantreis-fasen per email uit RAAS database."""
    try:
        from db import load_klantreis_fasen as _load
        return _load()
    except Exception:
        return {}
...
def save_klantreis_fasen(fasen: dict) -> bool:
    """Sla klantreis-fasen op naar RAAS database."""
    try:
        from db import save_klantreis_fasen as _save
        return _save(fasen)
    except Exception:
        return False
...
# Alternatieve kolomnamen die automatisch hernoemd worden
_PBI_COLUMN_ALIASES = {
    "Workspace name": "Pipedrive organisatie",
    "DisplayName": "Name",
}


def _normalize_pbi_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Hernoem alternatieve kolomnamen naar het verwachte formaat."""
    rename_map = {old: new for old, new in _PBI_COLUMN_ALIASES.items() if old in df.columns and new not in df.columns}
    if rename_map:
        df = df.rename(columns=rename_map)
    return df


def _map_workspace_to_org_names(df: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    """Vervang generieke workspace-namen (bv '1132 - Productie') door echte
    klantnamen uit de RAAS cache (bv '1132 - SDR Elektrotechniek').
    Returns (df, aantal_gemapped)."""
    if "Pipedrive organisatie" not in df.columns:
        return df, 0
    try:
        from db import load_powerbi_cache as _load
        cache_df, _, _ = _load()
        if cache_df is None or cache_df.empty or "Pipedrive organisatie" not in cache_df.columns:
            return df, 0
    except Exception:
        return df, 0

    # Bouw mapping: klantnummer -> volledige naam uit cache
    nr_to_name = {}
    for name in cache_df["Pipedrive organisatie"].dropna().unique():
        m = re.match(r"^(\d{4})", str(name))
        if m:
            nr_to_name[m.group(1)] = name
    ...
    def _replace(val):
        ...
        if m and m.group(1) in nr_to_name:
            replacement = nr_to_name[m.group(1)]
            if replacement != val:
                mapped += 1
            return replacement
        return val
...
def validate_new_pbi_excel(file_bytes: bytes) -> dict:
    ...
    try:
        ...
        return {"ok": False, "errors": [f"Kan Excel niet lezen: {e}"], "warnings": [], "df": None, "rows": 0, "orgs": 0}

    # Hernoem alternatieve kolomnamen
    had_workspace = "Workspace name" in df.columns and "Pipedrive organisatie" not in df.columns
    renamed = {old: new for old, new in _PBI_COLUMN_ALIASES.items() if old in df.columns and new not in df.columns}
    if renamed:
        df = _normalize_pbi_columns(df)
        warnings.append(f"Kolommen hernoemd: {', '.join(f'{o} -> {n}' for o, n in renamed.items())}")

    # Map generieke workspace-namen naar echte klantnamen via RAAS cache
    if had_workspace:
        df, n_mapped = _map_workspace_to_org_names(df)
        if n_mapped > 0:
            warnings.append(f"{n_mapped} rijen gekoppeld aan echte klantnamen uit cache")
...
def load_manual_bellijst() -> list:
    """Laad handmatig toegevoegde leads uit RAAS database."""
    try:
        from db import load_manual_bellijst as _load
        return _load()
    except Exception:
        return []
...
def save_manual_bellijst(entries: list) -> bool:
    """Sla handmatige bellijst op naar RAAS database."""
    try:
        from db import save_manual_bellijst as _save
...
def build_leads_df(ml_df, pd_df, web_mapping,
    ...
    Signalen: e-mail opens/clicks + deal fase.
    """
    deals = deals_dict or {}
    leads = []
    ...
    def _build_lead(email, name, company, phone, opened, clicked, pipedrive_match,
        ...
        total = open_score + click_score + deal_bonus + webinar_bonus + web_visit_score
